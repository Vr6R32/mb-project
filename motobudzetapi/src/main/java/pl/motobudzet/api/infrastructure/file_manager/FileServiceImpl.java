...
class FileServiceImpl implements FileService {
    ...
    public FileServiceImpl(@Lazy AdvertisementFacade advertisementFacade, PathsConfig pathsConfig, EntityManager entityManager) {
    ...
    public String verifySortAndSaveImages(UUID advertisementId, List<MultipartFile> files) {
        ...
        insertNewPhotos(advertisementId, uniqueFilenamesSet);

        return uniqueFilenamesSet.stream()
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("FILE_LIST_IS_EMPTY"));
    }
    ...
    private BufferedImage addLogoToImage(BufferedImage image, BufferedImage logo) {
        final double logoMaxWidthRatio = 0.25;
        final double logoMaxHeightRatio = 0.25;
